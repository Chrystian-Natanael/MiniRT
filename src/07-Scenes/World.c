...
/*   Updated: 2025/01/10 16:04:07 by tmalheir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Scenes.h"
#include "Objects.h" // Teste
...
// 		world->obj_lst->shape->create_shape(world->obj_lst->shape->id, world->obj_lst->shape);
// 	}
// 	else
// 	{
// 		node = (t_obj *)alloc_pool(sizeof(t_obj), set->objects);
// 		node->shape->create_shape(world->obj_lst->shape->id, node->shape);
...
t_world	*default_world(void)
{
	...
	lst_obj->shape->obj = (t_sp *)shape->obj;
	lst_obj->next = (t_obj *)alloc_pool(sizeof(t_obj), set->objects);
	lst_obj->next->shape->obj = (t_sp *)shape->obj;
...
// 	insert_into_light_list(&dfl_world->lights_lst, light1);
// 	lst_obj = (t_obj *)alloc_pool(sizeof(t_obj), set->objects);
// 	lst_obj->shape->create_shape(SPHERE, lst_obj->shape);
// 	lst_obj->next = (t_obj *)alloc_pool(sizeof(t_obj), set->objects);
// 	lst_obj->next->shape->create_shape(SPHERE, lst_obj->next->shape);
// 	lst_obj->shape->material.color = create_color(0.8, 1.0, 0.6);
// 	lst_obj->shape->material.diffu = create_color(0.7, 0.7, 0.7);
// 	lst_obj->shape->material.spec = create_color(0.2, 0.2, 0.2);
// 	lst_obj->prev = NULL;
